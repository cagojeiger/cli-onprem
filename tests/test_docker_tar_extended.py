...
def test_parse_image_reference_simple():
...
def test_parse_image_reference_with_namespace():
...
def test_parse_image_reference_with_registry():
    """Test parsing image reference with registry."""
    registry, namespace, image, tag = parse_image_reference("registry.example.com/image")
...
def test_parse_image_reference_full():
...
def test_generate_filename():
    ...
    filename = generate_filename(
        "docker.io", "library", "nginx", "latest", "amd64"
    )
    assert filename == "nginx__latest__amd64.tar"


def test_generate_filename_with_registry():
...
def test_generate_filename_with_namespace():
    """Test generating filename with non-default namespace."""
    filename = generate_filename(
        "docker.io", "user", "nginx", "latest", "amd64"
    )
...
def test_run_docker_command_success():
...
def test_run_docker_command_failure():
...
def test_docker_tar_save_stdout():
...
def test_docker_tar_save_with_output():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with mock.patch("cli_onprem.commands.docker_tar.check_image_exists") as mock_check:
            ...
            with mock.patch("cli_onprem.commands.docker_tar.run_docker_command") as mock_run:
                ...
                result = runner.invoke(
                    app, ["docker-tar", "save", "test:image", "--output", str(output_file)]
                )
