...
def test_generate_restore_script_with_purge():
                    ...
                    with mock.patch("builtins.open", mock.mock_open()):
                        with mock.patch("os.chmod"):
                            with mock.patch("glob.glob") as mock_glob:
                                mock_glob.return_value = ["testfile.pack/parts/0000.part"]
                                
                                with mock.patch("os.rename"):
                                    with mock.patch("os.remove"):
                                        with mock.patch("cli_onprem.commands.fatpack.get_file_size_mb") as mock_size:
                                            mock_size.return_value = 10
                                            
                                            result = runner.invoke(
                                                app, ["fatpack", "pack", str(test_file)]
                                            )
                                            
                                            assert result.exit_code == 0


def test_pack_command_directory():
                ...
                with mock.patch("pathlib.Path.is_dir") as mock_is_dir:
                    ...
                    with mock.patch("os.makedirs"):
                        with mock.patch("builtins.open", mock.mock_open()):
                            with mock.patch("os.chmod"):
                                with mock.patch("glob.glob") as mock_glob:
                                    mock_glob.return_value = ["testdir.pack/parts/0000.part"]
...
def test_restore_command():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with mock.patch("cli_onprem.commands.fatpack.run_command") as mock_run:
            ...
            result = runner.invoke(
                app, ["fatpack", "restore", str(pack_dir)]
            )
...
def test_restore_command_with_purge():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with mock.patch("cli_onprem.commands.fatpack.run_command") as mock_run:
            ...
            assert result.exit_code == 0
            mock_run.assert_called_once_with(["./restore.sh", "--purge"], cwd=str(pack_dir))


def test_restore_command_invalid_directory():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = runner.invoke(
            app, ["fatpack", "restore", str(non_existent_dir)]
        )
...
def test_restore_command_missing_script():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = runner.invoke(
            app, ["fatpack", "restore", str(empty_dir)]
        )
